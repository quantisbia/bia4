...
/* ─────────────────────────────────────────────────────────────────────────
   R12.25: OrganoidLabValueCard removido — plano descontinuado.
   Organoid Builder agora está incluso no plano Biofabricação 3D.
───────────────────────────────────────────────────────────────────────── */
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 animate-fadeIn">
      ...
      <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-3.5">
        ...
        <p className="text-xl font-bold text-white">
          {totalSpent > 0 ? `R$ ${((plan === "FREE" ? 0 : (PLANS.find(p => p.id === plan)?.price ?? LEGACY_PLAN_PRICES[plan] ?? 0)) / Math.max(1, totalSpent)).toFixed(2)}` : "—"}
        </p>
