...
import { BookOpen, Search, Loader2, ExternalLink, Tag, Sparkles } from "lucide-react"
...
export default function KnowledgePage() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="p-6 border-b border-white/5 shrink-0">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">{result.total.toLocaleString("pt-BR")} artigos científicos indexados</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-amber-500/40"
            />
          </div>

          <label className="flex items-center gap-2 cursor-pointer px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:border-amber-500/20 transition-colors">
            <input
              type="checkbox"
              checked={aiSearch}
            ...
            <span className="text-xs text-gray-300">Busca IA</span>
            <span className="text-[10px] text-gray-500">(1 crédito)</span>
          </label>
        </div>
      ...
      {/* Resumo IA */}
      {result.aiSummary && (
        <div className="mx-6 mt-4 p-4 rounded-xl bg-amber-500/5 border border-amber-500/15">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        ...
                        {article.citations > 0 && (
                          <span className="text-[10px] text-gray-600">{article.citations} citações</span>
                        )}
                      </div>
                      ...
                      {article.keywords?.length > 0 && (
                        <div className="flex items-center gap-2 mt-3 flex-wrap">
                          ...
                          {article.keywords.map((kw, i) => (
                            <button
                              key={i}
                              onClick={e => { e.stopPropagation(); setSearch(kw) }}
                              className="text-[10px] px-2 py-0.5 bg-white/5 rounded-md text-gray-500 hover:text-gray-300 transition-colors"
                            >
                              {kw}
                            </button>
                          ))}
